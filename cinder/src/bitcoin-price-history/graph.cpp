...
void BitcoinPriceHistory::setup() {
    ...
    auto content =
            downloader.download("https://poloniex.com/"
                                        "public?command=returnChartData&currencyPair=USDT_"
                                        "BTC&start=1496970103&end=9999999999&period=86400");
    ...
    double max =
            *std::max_element(std::begin(data["close"]), std::end(data["close"]));
    std::vector<float> pixelHeights;
    std::transform(std::begin(data["close"]), std::end(data["close"]),
